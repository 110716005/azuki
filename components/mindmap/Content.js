...
function Content(props) {
  ...
  return (
    <div className=" duration-500" style={{ position: 'absolute', willChange: 'transform, width, height opacity, translate3d', width: (isExpand ? "100%" : ((props.parentWidth - 30) * 0.25) + "px"), height: (isExpand ? "614px" : "360px"), zIndex: (isExpand ? "59" : "0"), transform: 'translate3d(6px, 6px, 0px' }}>
      {!isExpand ?
        (
          <div className={"bg-olive text-black h-full overflow-hidden duration-100 hover:shadow-lg " + (isExpand ? "" : "cursor-pointer")} onClick={reverseExpand}>
            <div className="flex group h-full flex-col justify-end p-6">
              ...
              <img class=" absolute opacity-50 group-hover:opacity-100 duration-300 transform  w-40 right-4 bottom-4 " src="https://www.azuki.com/map/beans.png" />
            </div>
          </div>
        ) :
        (
